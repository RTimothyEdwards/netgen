...
int PropertyCheckMismatch(struct objlist *tp1, struct nlist *tc1,
	char *inst1, struct objlist *tp2, struct nlist *tc2,
	char *inst2, int do_print, int *rval)
{
   int mismatches = 0;
   int len2, *check2;
...
int PropertyMatch(struct objlist *ob1, struct objlist *ob2, int do_print)
{
   ...
   while(1) {
	 ...
	 int multmatch = PropertyCheckMismatch(tp1, tc1, inst1,
			tp2, tc2, inst2, FALSE, NULL);
	 if (multmatch == 1) {
	 ...
	 mismatches += PropertyCheckMismatch(tp1, tc1, inst1,
			tp2, tc2, inst2, do_print, &rval);
      }
